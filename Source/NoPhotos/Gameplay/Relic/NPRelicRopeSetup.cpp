...
void ANPRelicRopeSetup::ReleaseRelicThroughGrabContract()
{
	...
	// BaseRelic을 수정하거나 protected ReleaseFromDisplay에 접근하지 않고,
	// 기존 공개 상호작용 경로를 통해 표시 상태와 물리를 함께 전환합니다.
	...
	if (!Grabbable || !RootPrimitive || !Grabbable->CanBeGrabbed())
	{
		UE_LOG(
			LogNoPhotos,
			Warning,
			TEXT("[%s] Could not release Relic physics through the grab contract. Relic=%s Grabbable=%s RootPrimitive=%s CanGrab=%s"),
			*GetNameSafe(this),
			*GetNameSafe(Relic),
	...
	if (!bSimulatingPhysics)
	{
		UE_LOG(
			LogNoPhotos,
			Warning,
			TEXT("[%s] Relic release signal completed, but the RootComponent is not simulating physics. Check the RelicMesh asset and physics collision settings."),
			*GetNameSafe(this));
	}
}
